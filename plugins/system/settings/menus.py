...
class SettingsMenu:
    ...
    async def _configure(self):
        config_menu = ConfigMenu(self._ctx, self._message, self._cur, self._timer)
...
class ConfigMenu:
    ...
    def __init__(self, ctx, msg, embed: SettingEmbed, timer):
        ...
        self._timer = timer
        self._components = [
            Button(style=ButtonStyle.gray, label="Back", id=ConfigMenu.InteractionTypes.BACK)
        ]

        if self._setting.type == SettingTypes.DISC_COMMAND:
            self._components.append(Button(style=ButtonStyle.green, label="Add ID Check", id=ConfigMenu.InteractionTypes.ADD_ID_CHECK))
            self._components.append(Button(style=ButtonStyle.red, label="Remove ID Check", id=ConfigMenu.InteractionTypes.REMOVE_ID_CHECK))
        elif self._setting.type == SettingTypes.MC_COMMAND:
            self._components.append(Button(style=ButtonStyle.green, label="Change OP Level", id=ConfigMenu.InteractionTypes.CHANGE_OP_LEVEL))

        if self._setting.config:
            self._components.append(Button(style=ButtonStyle.blue, label="Configure", id=ConfigMenu.InteractionTypes.CONFIGURE))
    ...
    async def start(self):
        ...
        while True:
            ...
            await self._timer.reset()
            interaction_type = interaction.raw_data["d"]["data"]["custom_id"]
            if interaction_type == ConfigMenu.InteractionTypes.BACK:
                await interaction.respond(type=InteractionType.DeferredUpdateMessage)
                break
    ...
    async def _add_id_check(self, interaction: Interaction):
        ...
        idx = 0 if not self._setting.config else 0
