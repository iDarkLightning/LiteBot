...
from ..middlewares.jwt import validate_jwt_headers, validate_jwt
...
@blueprint.websocket(RECV_EVENT_ROUTE)
async def _websocket(request: Request, socket: WebSocketCommonProtocol):
    async for message in socket:
        try:
            ...
            await socket.close(reason="Invalid data")
        except KeyError:
            await socket.close(reason="Invalid payload!")
